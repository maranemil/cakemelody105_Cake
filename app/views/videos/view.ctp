...
<div style="width: 640px; float: left; padding: 0 5px 5px 5px">
		...
		<h1 style="margin: 0px; color: white"><?=$Video[0]['videos']['bandname']?> - <?=$Video[0]['videos']['songtitle']?></h1>
		<div style="width: 630px; ">
			...
			<div id="layerFlash">
				<object width="630" height="376">
					...
					<embed src="http://www.youtube.com/v/<?=$Video[0]['videos']['tubecode']?>&hl=en" type="application/x-shockwave-flash" wmode="transparent" width="630" height="376">
					</embed>
				</object>
			</div>
			<!-- layerFlash / -->
			...
			<h3 class="videoInfoBox">
				...
				Youtube <A HREF="http://www.youtube.com/watch?v=<?=$Video[0]['videos']['tubecode']?>" target="_blank">( source )</A>
				<BR>
			</h3>
...
?>
			...
			<div class="videoBoxElem" style="">
				...
				<div style="clear: both"></div>
				<? $videoY = 'http://img.youtube.com/vi/'.$relVideo["videos"]["tubecode"].'/default.jpg'?>

				<?if(!file_exists(APP."webroot/imgvd/pic_thumb".$relVideo["videos"]["id"].".jpg")){?>
					
				<?}?>

				<div style="border:1px solid white; margin: 0 10px 10px 0; float: left">
					<img src='http://img.youtube.com/vi/<?=$relVideo["videos"]["tubecode"]?>/default.jpg'>
				</div>
		
				<div style="margin: 0px; float: left; ">

					<A HREF="<? echo $this->webroot."videos/view/".$relVideo["videos"]['id']."/".$relVideo["videos"]['bandname']; ?>">
						<?	if($relVideo["videos"]['recom']==1){?>
								<img src="<? echo $this->webroot?>img/heart.png" width=10 border="0">
						<?}?>
						<font style="margin: 0px; color: #333; font: bold 11px arial"><?php echo $relVideo["videos"]['bandname'].' - '.substr($relVideo["videos"]['songtitle'],0,25); ?> ...</font>
					</A><br /><br />
