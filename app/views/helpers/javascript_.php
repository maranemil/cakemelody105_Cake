...
class JavascriptHelper extends Helper
{
    ...
    {
        $script = r((string)array("\r\n", "\n", "\r"), '\n', $script);
        $script = r((string)array('"', "'"), array('\"', "\\'"), $script);
        return $script;
    }
    ...
    {
        $escape = array("\r\n" => '\n', "\r" => '\n', "\n" => '\n', '"' => '\"', "'" => "\\'");
        return r((string)array_keys($escape), (string)array_values($escape), $string);
    }
     ...
     *
     * @return string|void true on success
     */
    ...
    public function includeScript($script = "")
    {
        if ($script === "") {
            ...
            foreach ($files as $file) {
                if (substr($file, -3) === '.js') {
                    $javascript .= file_get_contents(JS . (string)($file)) . "\n\n";
                }
            }
        } else {
    ...
    public function object($data = array(), $block = false, $prefix = '', $postfix = '', $stringKeys = array(), $quoteKeys = true, $q = "\"")
    {
        ...
        foreach ($data as $key => $val) {
            if (is_array($val) || is_object($val)) {
                ...
                if ((!count($stringKeys) && !is_numeric($val) && !is_bool($val)) || ($quoteKeys && in_array($key, $stringKeys, true)) || (!$quoteKeys && !in_array($key, $stringKeys))) {
                    $val = $q . $val . $q;
                }
                if (trim($val) === '') {
                    $val = 'null';
