			...
			<div id="contentright">
				...
				<div class="sidebaruser">
					<ul class="sidebaruser">
						...
						<li><a href="<?=$this->webroot?>users/changepassword/<?=$session->read("User.id")?>">Change Password</a></li>

					<? if($session->read("User.rights")==15){ ?>
						<li><a href="<?=$this->webroot?>admins/index/<?=$session->read("User.id")?>">Administrator Videos</a></li>
					<?}?>
				...
				<div class="sidebarcat">
					...
					<?php foreach($this->requestAction('videos/menucategories/') AS $categ): ?>
						<li><a href="<?=$this->webroot?>videos/category/<?=$categ["categories"]["id"]?>" id=""><?=$categ["categories"]["name"]?></a></li>
					<?php endforeach; ?>
					</ul>
				</div>
			...
			<div id="footer">
				...
				<div id="footerright">
					...
					<p>
						...
						Idea is based on <?php echo $html->link("Pereira Pulido Nuno Ricardo Design","http://www.namaless.com"); ?>.<br />
